...
const generateProjectIdeas = async (userData, trends) => {
  ...
  const prompt = `
    ...
    - Target Protocol/Interests: ${userData.interests || 'General'}
...
/**
 * Generates a detailed technical specification for a project.
 */
const generateProjectSpec = async (title, description, techStack) => {
  const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
  const prompt = `
    Project: ${title}
    ...
    Generate a detailed technical specification in Markdown format.
    Include:
    1. **Core Features**: Bulleted list of MVP features.
    2. **Database Schema**: Simple text representation of key tables/collections.
    3. **API Endpoints**: Key API routes needed.
    4. **Implementation Steps**: High-level setup guide.
  `;
...
module.exports = { generateProjectIdeas, explainArchitecture, generateRoadmap, generateProjectSpec };
